...
Public Class year2_view
    Private Sub year2_view_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Populaterandr()
        Populatereplant()
        PopulatePWI()
        dateinspections()
    ...
    Private Sub dateinspections()
        ...
        Dim selectRecordQuery As String = "SELECT date_request, date_inspection  FROM year2nd_inspection_date WHERE quarter_code = @value ;"
        ...
        Using connection As New MySqlConnection(connectionString)
            Using command As New MySqlCommand(selectRecordQuery, connection)
                ...
                command.Parameters.AddWithValue("@value", year2_inspection.lblcode.Text)
    ...
    Private Sub Populaterandr()
        Dim connectionString As String = "server=localhost;username=root;password=;database=ngp"
        Dim selectRecordQuery As String = "SELECT commodity  FROM year2nd_data_tbl WHERE unique_code = @value AND type_code = @value1 AND quarter_code = @value2;"
    ...
    Private Sub PopulatePWI()
        ...
        Using connection As New MySqlConnection(connectionString)
            Using command As New MySqlCommand(selectRecordQuery, connection)
                ...
                command.Parameters.AddWithValue("@value2", year2_inspection.lblcode.Text) ' Use .Text.Trim() to get the value from the TextBox.
    ...
    Private Sub cmbcom_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbcom.SelectedIndexChanged
        ...
        Dim connectionString As String = "server=localhost;username=root;password=;database=ngp"
        Dim selectDataQuery As String = "SELECT quantity, unit, total_cost FROM year2nd_data_tbl WHERE commodity = @selectedCommodity AND unique_code = @uniqueCode AND type_code = @type AND quarter_code = @value1;"
        ...
        Using connection As New MySqlConnection(connectionString)
            Using command As New MySqlCommand(selectDataQuery, connection)
                ...
                command.Parameters.AddWithValue("@value1", year2_inspection.lblcode.Text)
                Try
                    connection.Open()
