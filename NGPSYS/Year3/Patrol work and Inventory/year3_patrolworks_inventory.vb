...
Public Class year3_patrolworks_inventory

    Dim totaltarget As Double
    Dim itcom As Integer
    Dim qty As Double
    Dim unit As Double
    Private Sub year3_patrolworks_inventory_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        PopulateComboBox()
        ...
        Using connection As New MySqlConnection(connectionString)
            Using command As New MySqlCommand(selectRecordQuery, connection)
                ...
                Try
                    ...
                    While reader.Read()
                        ' Add the commodity names to the list.
    ...
    Private Sub submit()
        ...
        Try
            ...
            Dim query As String = "INSERT INTO year3rd_data_tbl (unique_code, type_code, quarter_code, commodity, quantity, unit, total_cost, filter_code ) VALUES (@Value1, @Value2, @Value3, @Value4, @Value5, @Value6, @Value7, @Value8 )"
            Dim command As New MySqlCommand(query, connection)
            command.Parameters.AddWithValue("@Value1", uniquecodeloading)
            command.Parameters.AddWithValue("@Value2", lbltype.Text)
            command.Parameters.AddWithValue("@Value3", Year3_inspection.lblcode.Text)
