...
Public Class year3_1st2nd_quarter

    Dim survival As Decimal
    Private Sub year3_1st2nd_quarter_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnreplant_Click(sender As Object, e As EventArgs)
        ...
        Dim updateQuery As String = "UPDATE year3rd_inspection_date AS t1 " &
                               "INNER JOIN year3rd_data_tbl AS t2 ON t1.quarter_code = t2.quarter_code " &
    ...
    Private Sub AddSurvivalRateToolStripMenuItem_Click(sender As Object, e As EventArgs)
        finalsurvivalrate_frm.Show(maindash)
        Me.Hide()
    End Sub
    ...
    Public Sub start()
        ...
        If value1 >= 49 And value2 >= 49 Then
    ...
    Public Sub remark()
        ...
        Using connection As New MySqlConnection(connStr)
            Dim query As String = "SELECT remarks_id, remark FROM year3_remarks WHERE unique_code = @value AND quarter_code = @value1 AND filter_code = @value2;"
            Dim command As New MySqlCommand(query, connection)
            command.Parameters.AddWithValue("@value", uniquecodeloading)
    ...
    Private Sub txtseedproreport_TextChanged(sender As Object, e As EventArgs)
    ...
    Public Sub Datagridloadrwrs()
        ...
        Using connection As New MySqlConnection(connStr)
            ' SQL query with WHERE clause
            Dim query As String = "SELECT t1.date_request, t1.date_inspection, t1.quarter_code, t2.year3_data_id, t2.commodity, t2.quantity, t2.unit, t2.total_cost , t2.accomplished, t2.color " &
                     "FROM year3rd_inspection_date AS t1 " &
                     "INNER JOIN year3rd_data_tbl AS t2 ON t1.quarter_code = t2.quarter_code " &
                     "WHERE t2.unique_code = @value AND t2.type_code = 'rwrs' AND t2.filter_code = @value1;"
            Dim command As New MySqlCommand(query, connection)
            ...
            Using reader As MySqlDataReader = command.ExecuteReader()
                ...
                While reader.Read()
                    ...
                    row.Cells(0).Value = reader.GetString("year3_data_id")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnrwrs.Click
        ...
        Dim updateQuery As String = "UPDATE year3rd_inspection_date AS t1 " &
                                    "INNER JOIN year3rd_data_tbl AS t2 ON t1.quarter_code = t2.quarter_code " &
                                    "SET t2.accomplished = @value1, t2.color = @value2 " &
    ...
    Public Sub updatedata()
        ...
        Else
            Try
                ...
                ' Update plantation_estab_tbl
                Dim updateQuery As String = "UPDATE year3rd_data_tbl SET commodity = @Value2, quantity = @Value3, unit = @Value4, total_cost = @Value5 WHERE year3_data_id = @Value1"
                Using command As New MySqlCommand(updateQuery, connection)
        ...
        Else
            ...
            If result = DialogResult.Yes Then
                Try
                    ...
                    Dim deleteQuery As String = "DELETE FROM year3rd_data_tbl WHERE year3_data_id = @Value1"
    ...
    Public Sub Datagridloadreplanting()
        ...
        Using connection As New MySqlConnection(connStr)
            ' SQL query with WHERE clause
            Dim query As String = "SELECT t1.date_request, t1.date_inspection, t1.quarter_code, t2.year3_data_id, t2.commodity, t2.quantity, t2.unit, t2.total_cost, t2.planted " &
                         "FROM year3rd_inspection_date AS t1 " &
                         "INNER JOIN year3rd_data_tbl AS t2 ON t1.quarter_code = t2.quarter_code " &
                         "WHERE t2.unique_code = @value AND t2.type_code = 'replant' AND t2.filter_code = @value1;"
            Dim command As New MySqlCommand(query, connection)
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles btneditsurvival.Click
    ...
    Private Sub updatesurvival()
        ...
        Else
            Try
                ...
                ' Update plantation_estab_tbl
                Dim updateQuery As String = "UPDATE survival_rate_year3 SET survival_rate = @Value2 WHERE survival_id = @Value1"
                Using command As New MySqlCommand(updateQuery, connection)
        ...
        Else
            ...
            If result = DialogResult.Yes Then
                Try
                    ...
                    ' Delete record from plantation_estab_tbl
                    Dim deleteQuery As String = "DELETE FROM survival_rate_year3 WHERE survival_id = @Value1"
                    Using command As New MySqlCommand(deleteQuery, connection)
                        command.Parameters.AddWithValue("@Value1", txtidsurvival.Text)
    ...
    Private Sub survivalrating()
        ...
        Using connection As New MySqlConnection(connStr)
            ' SQL query with WHERE clause
            Dim query As String = "SELECT survival_id, survival_rate FROM survival_rate_year3 WHERE unique_code = @value AND filter_code = @value1;"
            Dim command As New MySqlCommand(query, connection)
    ...
    Private Sub btnreadd_Click(sender As Object, e As EventArgs) Handles btnreadd.Click
        ...
        Try
            ...
            Else
                ...
                Dim query As String = "INSERT INTO year3_remarks (unique_code, quarter_code, remark, filter_code) VALUES (@Value1, @Value2, @Value3, @Value4)"
                Dim command As New MySqlCommand(query, connection)
    ...
    Private Sub btnreplant_Click_1(sender As Object, e As EventArgs) Handles btnreplant.Click
        If String.IsNullOrEmpty(txtplanted.Text) Then
            MessageBox.Show("Please complete all fields.", "Incomplete Fields", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
    ...
    Private Sub AddSurvivalRateToolStripMenuItem_Click_1(sender As Object, e As EventArgs) Handles AddSurvivalRateToolStripMenuItem.Click
        finalsurvivalrate_frm.Show(maindash)
        Me.Hide()
    End Sub
    ...
    Public Sub updateRemarks()
        ...
        Try
            ...
            ' Update remarks
            Dim updateQuery As String = "UPDATE year3_remarks SET remark = @Value1 WHERE remarks_id = @Value2"
            Using command As New MySqlCommand(updateQuery, connection)
    ...
    Public Sub deleteRemarks()
        ...
        Try
            connection.Open()
            ' Delete remark
            Dim deleteQuery As String = "DELETE FROM year3_remarks WHERE remarks_id = @Value1"
            Using command As New MySqlCommand(deleteQuery, connection)
    ...
    Dim connStr As String = "server=localhost;username=root;password=;database=ngp"
    Public Sub LoadReportquarter12()
        ...
        Try
            year3_quarter12_Report.ReportViewer1.RefreshReport()

            ' Load report files
            With year3_quarter12_Report.ReportViewer1.LocalReport
                .ReportPath = Application.StartupPath & "\report\year3_quarter12_MP.rdlc"
                .DataSources.Clear()
            End With
            ...
            Using connection As New MySqlConnection(connStr)
                ...
                daDetails.Fill(dsDetails, "quarter12dbrwrs")
                ...
                daReplanting.Fill(dsReplanting, "quarter12dbreplanting")

                ' Query for patrol
                Dim querypatrol As String = "SELECT t2.commodity, t2.quantity, t2.unit, t2.total_cost, t2.planted, t2.status " &
                            "FROM year3rd_data_tbl t2 " &
                            "INNER JOIN (" &
                            "    SELECT DISTINCT t2_inner.status " &
                            "    FROM year3rd_inspection_date t1 " &
                            "    INNER JOIN year3rd_data_tbl t2_inner ON t1.quarter_code = t2_inner.quarter_code " &
                ...
                daSurvival.Fill(dsSurvival, "quarter12dbsurvival")
                ...
                rptDSDetails = New ReportDataSource("quarter12DB", dsDetails.Tables("quarter12dbrwrs"))
                rptDSReplanting = New ReportDataSource("quarter12DBreplanting", dsReplanting.Tables("quarter12dbreplanting"))
                rptDSPatrol = New ReportDataSource("quarter12DBpatrol", dsPatrol.Tables("quarter12dbpatrol"))
                rptDSSurvival = New ReportDataSource("quarter12DBsurvival", dsSurvival.Tables("quarter12dbsurvival"))
                year3_quarter12_Report.ReportViewer1.LocalReport.DataSources.Add(rptDSDetails)
                year3_quarter12_Report.ReportViewer1.LocalReport.DataSources.Add(rptDSReplanting)
                year3_quarter12_Report.ReportViewer1.LocalReport.DataSources.Add(rptDSPatrol)
                year3_quarter12_Report.ReportViewer1.LocalReport.DataSources.Add(rptDSSurvival)
            End Using

            ' Set display mode and zoom level
            year3_quarter12_Report.ReportViewer1.SetDisplayMode(Microsoft.Reporting.WinForms.DisplayMode.PrintLayout)
            year3_quarter12_Report.ReportViewer1.ZoomMode = ZoomMode.Percent
            year3_quarter12_Report.ReportViewer1.ZoomPercent = 30
    ...
    Public Sub parameters()
        Try
            ...
            year3_quarter12_Report.ReportViewer1.LocalReport.ReportPath = Application.StartupPath & "\report\year3_quarter12_MP.rdlc"

            ' Set the parameters to the ReportViewer control
            year3_quarter12_Report.ReportViewer1.LocalReport.SetParameters(reportParameters)
    ...
    Private Sub btnprintreport_Click(sender As Object, e As EventArgs) Handles btnprintreport.Click
        LoadReportquarter12()
        parameters()
        year3_quarter12_Report.ShowDialog()
    End Sub
    ...
    Private Sub txtreid_TextChanged(sender As Object, e As EventArgs) Handles txtreid.TextChanged
